...
4. **Tickets.** Once a bb release with bb account is out and stable, change
   `TUNNEL_TICKETS` to `"on"` in the production `vars` and deploy that version
   the same way. bb builds with bb account then log `tunnel connected with a
   ticket`; a bb whose ticket the gate refuses dials with its credential.
...
showed before the deployment. Rolling back never locks a bb out: bb builds with
bb account dial with the raw credential whenever the gate refuses their ticket.
`wrangler rollback` closed connected tunnels cleanly in every case we saw,
