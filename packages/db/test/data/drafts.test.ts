...
describe("draft resources", () => {
  ...
  it("retains saved draft attachments until their final draft reference is removed", () => {
    const project = createProjectFixture(db);
    db.insert(projectAttachmentBackfills).values({
    ...
    const attachment = recordProjectAttachment(db, {
      projectId: project.id,
      storedPath: "saved.txt",
    ...
    const content = draftInput({
      projectId: project.id,
      payloadJson: JSON.stringify({ prompt: { attachments: [{ path: attachment.storedPath }] } }),
    });
    const first = createDraft(db, content);
    const second = createDraft(db, { ...content, id: "draft_two" });
    const now = PROJECT_ATTACHMENT_GRACE_MS + 1;
    expect(claimProjectAttachments(db, project.id, now)).toEqual([]);
    deleteDraft(db, { id: first.id, expectedRevision: first.revision });
    expect(claimProjectAttachments(db, project.id, now)).toEqual([]);
    updateDraft(db, {
      ...content,
      id: second.id,
      expectedRevision: second.revision,
      payloadJson: JSON.stringify({ prompt: { attachments: [] } }),
    });
    expect(claimProjectAttachments(db, project.id, now)).toEqual([
      { ...attachment, deletionClaimedAt: now },
    ]);
  });
