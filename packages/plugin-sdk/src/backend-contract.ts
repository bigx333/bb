...
export interface PluginHosts {
   ...
   * resulting set to that host's daemon. The server only delivers to an
   * enrolled host whose daemon reports its machine credential; while a host
   * is offline, not enrolled, or missing that credential, the declaration is
   * retained and delivered on its next credentialed daemon session. Throws
   * only for a host that does not exist or was removed. Tunnel identity is
   * deliberately not accepted here: it is owned by the daemon's trusted
   * enrollment.
   */
